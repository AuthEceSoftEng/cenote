...
describe("Test /median route", () => {
	...
	test("can calculate correct median value", async () => {
		...
		expect(response.body.results[0].median).toBe(median(((n) => [...new Array(n).keys()])(NUM_OF_DOCS + 1).slice(1)));
	});
	...
	test("query measurements with filter property (1)", async () => {
		...
		expect(response.statusCode).toBe(200);
		expect(response.body.ok).toBe(true);
		expect(response.body.results[0].median).toBe(median(((n) => [...new Array(n).keys()])(NUM_OF_DOCS + 1).slice(Math.ceil(NUM_OF_DOCS / 2))));
	});
