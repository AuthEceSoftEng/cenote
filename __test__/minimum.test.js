...
const eventCollection = "minimum";

describe("Test /minimum route", () => {
	afterAll(async () => {
		const query = {
			eventCollection,
		};
	...
	test("query without specifying target_property property fails", async () => {
		...
		const response = await got.get(`/projects/${PROJECT_ID}/queries/minimum`, { query });
	...
	test("can calculate correct minimum value", async () => {
		const query = {
			masterKey: CENOTE_MASTER_KEY,
			event_collection: eventCollection,
			target_property: "a",
		...
		expect(response.body.results[0].min).toBe(1);
	});
	...
	test("query measurements with existing group_by property", async () => {
		...
		response.body.results.forEach((res) => expect(Object.keys(res)).toEqual(["c", "min"]));
	});
	...
	test("query measurements with filter property (1)", async () => {
		...
		expect(response.statusCode).toBe(200);
		expect(response.body.ok).toBe(true);
		expect(response.body.results[0].min).toBe(Math.ceil(NUM_OF_DOCS / 2));
	});
	...
	test("query measurements with filter property (2)", async () => {
		...
		expect(response.statusCode).toBe(200);
		expect(response.body.ok).toBe(true);
		expect(response.body.results[0].min).toBe(NUM_OF_DOCS);
	});
	...
	test("query measurements with filter property (3)", async () => {
		...
		expect(response.statusCode).toBe(200);
		expect(response.body.ok).toBe(true);
		expect(response.body.results[0].min).toBe(null);
	});
});
